...
#' the type of report (confirmed vs suspected cases), the version of the report (i.e. the date of publication),
...
#' Engineering. It should be noted, however, that the open data format of the mexican official report is generated by
#' third parties, since the original report is a PDF document.
#' Due to the wide variety of data sources and formats, not all parameter combinations are possible. Please consider the following table.
#' Each column is a valid parameter value given the data scope.
#'
#' \tabular{lcccc}{
...
#' \code{Mexico} \tab \code{Guzmart} \tab \code{confirmed} \tab \code{today} or any date from 16/03/2020 onwards. \tab \code{TRUE}, \code{FALSE} \cr
...
#' }
#'
#' For example, when requesting data for Mexico, you can either use \code{Serendipia} or \code{Guzmart}
#' as \code{source} parameters. If you choose \code{Guzmart},
#' you can only use \code{type="confiremd"} and any date from from 16/03/2020 onwards.
#'
