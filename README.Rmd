...
* DESCRIPTION and NAMESPACE file now has propper package dependencies (under CRAN rules). 
...
* Parameter `neat` in `getData`, `GetFromSerendipia`, and `GetFromGuzmart` functions. This parameter allows user to decide if some data cleaning is performed on the returned `tibbles` by `GetFromSerendipia` and `GetFromGuzmart`. Specially, propper date parsing and state name capitalizing. 
