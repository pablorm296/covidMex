...
* `GetFromSerendipia` broke after changes in Serendipia's report naming pattern. Now, function searches for date pattern; when found, looks for link to relevant file in nearby html nodes by matching some basic regex.
...
* Official Mexico's Ministry of Health (Secretaría de Salud) open dataset is now available. As any other data source, user can get data using `getData` wrapper, or directly using `GetFromSSA`.
* Function tests.
